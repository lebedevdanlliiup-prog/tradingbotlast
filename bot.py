...
async def send_tg(chat_id, text, reply_markup=None, delete_previous=False):
    if delete_previous and chat_id in last_menu_msg_id and last_menu_msg_id[chat_id]:
        await delete_message(chat_id, last_menu_msg_id[chat_id])
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(url, json=payload, timeout=10) as resp:
                data = await resp.json()
                if data.get("ok") and data.get("result"):
                    last_menu_msg_id[chat_id] = data["result"]["message_id"]
        except:
            pass
...
def get_pair_selection_keyboard(chat_id):
    ...
    page = pair_page.get(chat_id, 0)
    pairs_per_page = 15
    total_pages = (len(all_pairs) + pairs_per_page - 1) // pairs_per_page
    
    start = page * pairs_per_page
    end = min(start + pairs_per_page, len(all_pairs))
    page_pairs = all_pairs[start:end]
    ...
    for i, pair in enumerate(page_pairs):
        ...
        row.append({"text": text, "callback_data": f"toggle_{pair}"})
        if len(row) == 2:
            buttons.append(row)
            row = []
    if row:
    ...
    nav_row = []
    if page > 0:
        nav_row.append({"text": "◀️", "callback_data": "prev_page"})
    if page < total_pages - 1:
        nav_row.append({"text": "▶️", "callback_data": "next_page"})
    if nav_row:
...
async def handle_callback(callback_query):
    ...
    if data == "back_main":
        ...
        await send_tg(chat_id, "Main menu:", kb, delete_previous=True)

    elif data == "exit_pairs":
        if chat_id in temp_pairs:
            del temp_pairs[chat_id]
        if chat_id in pair_page:
            del pair_page[chat_id]
        current_thresh = user_thresholds.get(chat_id, DEFAULT_MIN_PROBABILITY)
        kb = get_main_menu_keyboard(CURRENT_MODE, current_thresh, is_notifications_enabled(chat_id))
        await send_tg(chat_id, "Main menu:", kb, delete_previous=True)
        ...
        await send_tg(chat_id, text, kb, delete_previous=True)

    elif data == "prev_page":
        if chat_id in pair_page and pair_page[chat_id] > 0:
            pair_page[chat_id] -= 1
        kb = get_pair_selection_keyboard(chat_id)
        pairs = temp_pairs.get(chat_id, user_pairs.get(chat_id, []))
        text = f"Current selection ({len(pairs)}):\n\n" + "\n".join([f"{i+1}. {p}" for i, p in enumerate(pairs)]) + "\n\nClick to toggle, then press SAVE."
        await send_tg(chat_id, text, kb, delete_previous=True)
        ...
        await send_tg(chat_id, "Enter number (10-99) for minimum probability threshold:", get_threshold_input_keyboard(), delete_previous=True)
        ...
        await send_tg(chat_id, info, get_back_menu_keyboard(), delete_previous=True)
...
async def handle_text(message):
    ...
    if chat_id in waiting_for_threshold:
        await delete_message(chat_id, user_msg_id)
        ...
        try:
            ...
            if 10 <= val <= 99:
                ...
                await send_tg(chat_id, "Enter value between 10 and 99:", get_threshold_input_keyboard(), delete_previous=True)
        except:
            await send_tg(chat_id, "Please enter a valid number:", get_threshold_input_keyboard(), delete_previous=True)
        return
    ...
    if text == "/start":
        ...
        await send_and_delete(chat_id, "🚀 Bot is starting...", delete_after=3)
        ...
        try:
            ...
            for user_id in list(active_users):
                if is_notifications_enabled(user_id):
                    await send_and_delete(user_id, "🔍 Scanning market...", delete_after=3)
