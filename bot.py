    ...
    for row in buttons:
        for btn in row:
            ...
            if btn_text == "Strong" and mode == "Strong":
                ...
                btn["text"] = "🔸 All"
            elif btn_text in ["Strong", "Meme", "All"]:
                btn["text"] = btn_text
...
async def show_notification_status(chat_id, status):
    if status:
        msg = "🔔 Bot is turning ON..."
    else:
        msg = "🔕 Bot is turning OFF..."
...
async def send_status_message():
    """Отправляет сообщение о работе бота каждые 3 минуты как обычное сообщение"""
    while BOT_RUNNING:
        ...
        for user_id in list(active_users):
            ...
            if current_time - last_time >= STATUS_MESSAGE_INTERVAL:
                ...
                await send_tg(msg, user_id)  # Обычное сообщение, без удаления
                last_status_time[user_id] = current_time
                log(f"Status message sent to {user_id}")
