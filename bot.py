...
last_scan_time = 0
last_no_signal_msg = {}
last_menu_msg_id = {}  # chat_id -> message_id
...
async def send_signal(chat_id, text):
    """Отправляет сигнал как новое сообщение (не редактирует меню)"""
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    payload = {"chat_id": chat_id, "text": text, "parse_mode": "HTML"}
    async with aiohttp.ClientSession() as session:
        try:
...
async def handle_callback(callback_query):
    ...
    if data == "back_main":
        ...
        await edit_or_send(chat_id, "Enter number (10-99) for minimum probability threshold:")
    ...
    if chat_id in waiting_for_threshold:
        try:
            ...
            if 10 <= val <= 99:
                ...
                await edit_or_send(chat_id, "Enter value between 10 and 99.")
        except:
            await edit_or_send(chat_id, "Please enter a number.")
        return
