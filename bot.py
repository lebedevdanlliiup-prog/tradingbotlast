...
last_no_signal_msg = {}
last_menu_msg_id = {}
threshold_msg_id = {}  # chat_id -> message_id (сообщение с запросом порога)
...
async def edit_or_send(chat_id, text, reply_markup=None):
    if chat_id in last_menu_msg_id and last_menu_msg_id[chat_id]:
        try:
            url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/editMessageText"
            payload = {
                "chat_id": chat_id,
                "message_id": last_menu_msg_id[chat_id],
                "text": text,
                "parse_mode": "HTML"
            }
            if reply_markup:
...
async def send_threshold_request(chat_id, text, reply_markup=None):
    """Отправляет сообщение с запросом порога и сохраняет его ID"""
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
    payload = {"chat_id": chat_id, "text": text, "parse_mode": "HTML"}
    if reply_markup:
        payload["reply_markup"] = reply_markup
    async with aiohttp.ClientSession() as session:
        try:
            async with session.post(url, json=payload, timeout=10) as resp:
                data = await resp.json()
                if data.get("ok") and data.get("result"):
                    threshold_msg_id[chat_id] = data["result"]["message_id"]
        except:
            pass
...
async def handle_callback(callback_query):
    ...
    if data == "back_main":
        ...
        await edit_or_send(chat_id, "Main menu:", kb)
        ...
        await edit_or_send(chat_id, text, kb)
        ...
        await send_threshold_request(chat_id, "Enter number (10-99) for minimum probability threshold:", get_threshold_input_keyboard())
        ...
        await edit_or_send(chat_id, info, get_back_menu_keyboard())
...
async def handle_text(message):
    ...
    if chat_id in waiting_for_threshold:
        ...
        await delete_user_message(chat_id, user_msg_id)
        
        # Удаляем сообщение с запросом порога
        if chat_id in threshold_msg_id and threshold_msg_id[chat_id]:
            await delete_message(chat_id, threshold_msg_id[chat_id])
            threshold_msg_id[chat_id] = None
        ...
        try:
            ...
            if 10 <= val <= 99:
                ...
                # Если число не в диапазоне — показываем новый запрос
                waiting_for_threshold[chat_id] = True
                await send_threshold_request(chat_id, "Enter value between 10 and 99:", get_threshold_input_keyboard())
        except:
            waiting_for_threshold[chat_id] = True
            await send_threshold_request(chat_id, "Please enter a valid number:", get_threshold_input_keyboard())
        return
    ...
    if text == "/start":
        ...
        current_thresh = user_thresholds.get(chat_id, DEFAULT_MIN_PROBABILITY)
        kb = get_main_menu_keyboard(CURRENT_MODE, current_thresh, is_notifications_enabled(chat_id))
        await edit_or_send(chat_id, "Welcome! Use the menu below:", kb)
