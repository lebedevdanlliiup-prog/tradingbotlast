...
async def send_new_menu(chat_id, text, reply_markup=None):
    """Отправляет новое меню и сохраняет его ID"""
    url = f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/sendMessage"
...
async def handle_callback(callback_query):
    ...
    if data == "back_main":
        ...
        await send_new_menu(chat_id, "Main menu:", kb)
        ...
        await send_new_menu(chat_id, text, kb)
        ...
        await send_new_menu(chat_id, info, kb)
...
async def handle_text(message):
    ...
    if text == "/start":
        ...
        current_thresh = user_thresholds.get(chat_id, DEFAULT_MIN_PROBABILITY)
        kb = get_main_menu_keyboard(CURRENT_MODE, current_thresh, is_notifications_enabled(chat_id))
        await send_new_menu(chat_id, "Welcome! Use the menu below:", kb)
