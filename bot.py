...
async def handle_text(message):
    ...
    if text == "/start":
        ...
        # Приветствие, которое удалится через 10 секунд
        await send_and_delete(chat_id, "✅ Бот активирован! Сканирую рынок...\n⏳ Ожидайте сигналы.", delete_after=10)
...
async def polling_loop():
    ...
    while True:
        try:
            ...
            await asyncio.sleep(1)  # Ускорено с 2 до 1 секунды
        except:
            await asyncio.sleep(5)  # Ускорено с 10 до 5 секунд
    ...
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                ...
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 Пинг")
        except:
            pass
