...
async def keep_alive():
    """Каждые 2 минуты пингует самого себя, чтобы Render не усыпил"""
    while True:
        try:
            async with aiohttp.ClientSession() as session:
                await session.get(f"http://localhost:{PORT}/ping", timeout=5)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 Self-ping")
        except:
            print(f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️ Self-ping failed")
