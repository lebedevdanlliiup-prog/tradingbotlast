    ...
    for row in buttons:
        for btn in row:
            if btn["text"].startswith("✅"):
                continue
            btn_name = btn["text"].split()[0] if btn["text"].split()[0] in ["Strong", "Meme", "All"] else ""
            if mode == btn_name and btn["text"] not in ["✅ Strong", "✅ Meme", "✅ All"]:
                btn["text"] = f"✅ {btn['text']}"
...
def get_pair_selection_keyboard(chat_id):
    ...
    buttons.append([{"text": "EXIT", "callback_data": "back_main"}])
    return {"inline_keyboard": buttons}
...
async def handle_callback(callback_query):
    ...
    if data == "back_main":
        ...
        save_data()
        await send_tg(f"Pairs saved: {len(pairs)} selected.", chat_id, delete_previous=True)
        ...
        await send_tg(f"Notifications: {status_emoji} {status_text}", chat_id, delete_previous=True)
        ...
        await send_tg("Enter number (10-99) for minimum probability threshold:", chat_id, delete_previous=True)
        ...
        await send_tg(info, chat_id, delete_previous=True)
